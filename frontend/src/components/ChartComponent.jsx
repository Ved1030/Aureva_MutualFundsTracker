...
export default function ChartComponent({ data }) {
  ...
  const { displayedData, isFallback, formatLabel } = useMemo(() => {
    ...
    if (filtered.length >= 2) {
      ...
      console.log(`[Chart] ${userFilter}: ${filtered.length}/${sortedData.length} records (cutoff ${cutoff.toISOString().split('T')[0]})`);
      return {
  ...
  console.log('[Chart] Selected Range:', userFilter);
  console.log('[Chart] Displayed Range:', formatLabel, isFallback ? '(fallback - max available)' : '');
  console.log('[Chart] Total NAV Records:', sortedData.length);
  console.log('[Chart] Filtered NAV Records:', displayedData.length);
  console.log('[Chart] Start Date:', displayedData.length > 0 ? displayedData[0].date.toISOString().split('T')[0] : 'N/A');
  console.log('[Chart] End Date:', displayedData.length > 0 ? displayedData[displayedData.length - 1].date.toISOString().split('T')[0] : 'N/A');

  if (!data || data.length === 0 || sortedData.length < 2) {
    return (
      <div className="flex flex-col items-center justify-center h-80 text-slate-400 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 rounded-xl p-1">
          {FILTERS.map((f) => (
            <button
              ...
              className={`range-btn px-4 py-1.5 text-sm font-semibold rounded-lg transition-all ${
                userFilter === f.label
                  ? 'range-btn-active'
      ...
      <div className="flex items-center gap-6 mb-6 px-1">
        {latestNav && (
          <div>
            ...
            <p className="text-xl font-bold text-slate-800">₹{latestNav.toFixed(4)}</p>
          </div>
        )}
        {oldestNav && (
          <div>
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">Range Start</p>
            <p className="text-xl font-bold text-slate-800">₹{oldestNav.toFixed(4)}</p>
          </div>
        )}
        {growth !== null && (
          <div>
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">Growth</p>
            <p className={`text-xl font-bold flex items-center gap-1 ${growth >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>

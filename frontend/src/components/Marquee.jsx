...
export default function Marquee() {
  ...
  return (
    <div className="bg-slate-900 border-b border-slate-800 overflow-hidden h-10">
      <div className="relative flex items-center h-full">
        <div className="flex items-center gap-2 px-4 bg-indigo-600 text-white text-xs font-semibold uppercase tracking-wider h-full z-10 relative shrink-0">
        ...
        <div className="overflow-hidden flex-1 h-full relative">
          <div className="flex items-center h-full marquee-track whitespace-nowrap" style={{ width: 'fit-content' }}>
            {doubled.map((name, i) => (
              <span
                key={`${name}-${i}`}
                className="inline-flex items-center gap-2 px-6 text-sm text-slate-300 border-r border-slate-700/50 h-full"
              >

...
function VariantRow({ fund, watchlistSet, adding, onAdd, onView }) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2.5 hover:bg-slate-50 transition-colors rounded-lg">
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <span className="w-1.5 h-1.5 rounded-full bg-indigo-300 shrink-0" />
        <span className="text-sm text-slate-600 font-medium truncate">
          {fund.variantLabel || fund.schemeName}
        </span>
        <span className="text-xs text-slate-400 font-mono shrink-0 hidden sm:inline">#{fund.schemeCode}</span>
      </div>
      <div className="flex items-center gap-2 shrink-0 ml-2">
        <button
          onClick={() => onView(fund.schemeCode)}
          className="px-3 py-1.5 text-xs font-semibold text-indigo-600 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition-all active:scale-[0.97]"
        >
          View
        </button>
        {!isInWatchlist ? (
          <button
            ...
            className="px-3 py-1.5 text-xs font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-all active:scale-[0.97]"
          >
            {adding === fund.schemeCode ? '...' : '+ Add'}
          </button>
        ) : (
          <span className="px-3 py-1.5 text-xs font-semibold text-emerald-600 bg-emerald-50 rounded-lg flex items-center gap-1">
...
export default function FundCard({ group, watchlistSet, adding, onAdd, onView }) {
  ...
  return (
    <div className="fund-card bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      {!hasDuplicates && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 px-5 pb-5 pt-3 border-t border-slate-100">
          <button
          ...
          {!watchlistSet?.has?.(variants[0].schemeCode) ? (
            <button
              ...
              className="flex-1 px-4 py-3 sm:py-2.5 text-sm font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed btn-primary active:scale-[0.98]"
            >
              {adding === variants[0].schemeCode ? 'Adding...' : '+ Add to Watchlist'}
            </button>
          ) : (
            <div className="flex-1 px-4 py-3 sm:py-2.5 text-sm font-semibold text-emerald-600 bg-emerald-50 rounded-xl text-center flex items-center justify-center gap-1.5">
